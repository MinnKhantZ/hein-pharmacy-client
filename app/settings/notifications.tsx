...
export default function NotificationsScreen() {
  ...
  const [timePickerTarget, setTimePickerTarget] = useState<
    "lowStockAlertTime" | "expiryAlertTime"
  >("lowStockAlertTime");
  ...
  const openTimePicker = () => {
    setTimePickerTarget("lowStockAlertTime");
    setShowTimePicker(true);
  };

  const openExpiryTimePicker = () => {
    setTimePickerTarget("expiryAlertTime");
    setShowTimePicker(true);
  };

  const handleExpiryDaysChange = (text: string) => {
    const parsed = parseInt(text, 10);
    setTempNotificationSettings({
      ...tempNotificationSettings,
      expiryAlertDaysBefore: Number.isNaN(parsed) ? 0 : Math.max(0, parsed),
    });
  };
          ...
          {tempNotificationSettings.expiryAlerts && (
            <View style={styles.timePickerContainer}>
              ...
              <TextInput
                ...
                value={String(tempNotificationSettings.expiryAlertDaysBefore)}
                onChangeText={handleExpiryDaysChange}
                keyboardType="number-pad"
                placeholder={t("Days before expiry")}
              />

              <Text style={[styles.timePickerLabel, { marginTop: 12 }]}>
                {t("Expiration Alert Time")}
              </Text>
              <TouchableOpacity
                style={styles.timePickerButton}
                onPress={openExpiryTimePicker}
              >
                <Text style={styles.timePickerText}>
                  {formatTimeForDisplay(
                    tempNotificationSettings.expiryAlertTime || "09:00",
                  )}
                </Text>
                <Text style={styles.timePickerIcon}>🕐</Text>
              </TouchableOpacity>
      ...
      {showTimePicker && (
        <DateTimePicker
          value={timeStringToDate(
            (timePickerTarget === "lowStockAlertTime"
              ? tempNotificationSettings.lowStockAlertTime
              : tempNotificationSettings.expiryAlertTime) || "09:00",
          )}
          mode="time"
